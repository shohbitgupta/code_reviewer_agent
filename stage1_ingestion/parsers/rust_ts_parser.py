...
# Standard-library macros that should not be recorded as cross-file CALLS edges
RUST_STDLIB_MACROS: Set[str] = {
    "println", "eprintln", "print", "eprint",
    "vec", "format", "write", "writeln",
    "assert", "assert_eq", "assert_ne", "debug_assert",
    "debug_assert_eq", "debug_assert_ne",
    "panic", "todo", "unimplemented", "unreachable",
    "dbg", "include", "include_str", "include_bytes",
...
class RustTsParser(TreeSitterParser):
    ...
    # ── Call collection (overrides base to add macro_invocation support) ─────
    ...
    def _collect_calls(
        ...
        """Collect call_expression AND macro_invocation callees from *node*."""
        seen:   list = []
        result: list = []
        ...
        def _walk(n):
            if n.type == "call_expression":
                ...
                # e.g.  log::info!("…")  or  my_macro!(…)
                macro_node = n.child_by_field_name("macro") or (
                    n.children[0] if n.children else None
                )
                if macro_node is not None:
                    raw  = cls._node_text(macro_node, source_bytes).strip().rstrip("!")
                    # Take rightmost component for path macros: "log::info" → "info"
                    name = raw.split("::")[-1].strip()
                    if (
                        name
                        and name not in RUST_STDLIB_MACROS
