...
GAP_FILL_THRESHOLD   = 20   # fill gaps of this many uncovered lines or more
MODULE_PREVIEW_MAX   = 50   # max lines captured in MODULE chunk preamble

# Preamble patterns: lines that are imports, comments, package declarations, etc.
_PREAMBLE_RE = re.compile(
    r"^\s*(?:import|from|use\s|#|//|/\*|\*|package\s|@file|library\s|part\s)"
)
...
class HierarchicalChunkBuilder:
    ...
    def _module_chunk(self, meta: FileMeta, lines: List[str]) -> CodeChunk:
        """FILE-LEVEL chunk: header + full import/preamble block (up to 50 lines).

        Scans forward from line 1 until the first non-preamble, non-blank line
        to capture the entire import block rather than a hard-coded 10 lines.
        """
        preamble_end = 10  # safe default
        for idx, line in enumerate(lines[:MODULE_PREVIEW_MAX]):
            if line.strip() and not _PREAMBLE_RE.match(line):
                # Include one extra line for context (e.g. first class declaration)
                preamble_end = min(idx + 2, MODULE_PREVIEW_MAX)
                break
        else:
            preamble_end = min(len(lines), MODULE_PREVIEW_MAX)

        preview = lines[:preamble_end]
        content = (
        ...
        return CodeChunk.new(
            ...
            start_line  = 1,
            end_line    = max(1, min(preamble_end, len(lines))),
            content     = content,
        )
    ...
    def _ast_chunks(self, parsed_file: ParsedFile) -> List[CodeChunk]:
        ...
        for sym in parsed_file.symbols:
            ...
            # Prepend a file-location comment so the code_vector embedding
            # carries file identity — improves cross-file retrieval precision.
            if chunk_type != ChunkType.IMPORT:
                content = f"# {meta.file_path}:{sym.start_line}\n{sym.source}"
            else:
                content = sym.source
    ...
    def _split_large_symbol(
        ...
        Attempts to split at a blank line near the nominal boundary (±10 lines)
        to avoid fragmenting if/else blocks or try/except handlers.
        """
        chunks: List[CodeChunk] = []
        ...
        while i < end:
            ...
            nominal_end = min(i + MAX_FUNCTION_LINES, end)

            # Search ±10 lines from nominal_end for a blank line (semantic boundary)
            chunk_end = nominal_end
            if nominal_end < end:  # only seek semantic boundary if not at the symbol end
                for offset in range(0, 11):
                    found = False
                    for candidate in (nominal_end - offset, nominal_end + offset):
                        if i < candidate < end and not lines[candidate - 1].strip():
                            chunk_end = candidate
                            found = True
                            break
                    if found:
            ...
            i = chunk_end - WINDOW_OVERLAP_LINES
    ...
    def _gap_fill(
        ...
        """Apply sliding window to uncovered line ranges >= GAP_FILL_THRESHOLD lines."""
        if not ast_chunks:
