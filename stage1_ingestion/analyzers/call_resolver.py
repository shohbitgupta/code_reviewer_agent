...
from stage1_ingestion.analyzers.base import AnalysisResult, ResolvedCall
...
class Resolver:
    ...
    def _strip_receiver_prefix(name: str) -> str:
        """Strip common receiver prefixes from a Kotlin/Dart/Swift/Python call name.

        Handles:
          - Simple prefixes: self., cls., this., super.
          - Null-safe receivers: this?., super?.
          - Kotlin lambda implicit receiver: it.
          - Chained super: super.super.method → method
        """
        # Simple prefix strip (most common path)
        for prefix in ("self.", "cls.", "this?.", "this.", "super?.", "super.", "it."):
            if name.startswith(prefix):
                name = name[len(prefix):]
                break

        # Handle residual chained super: "super.method" after first strip
