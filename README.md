...
- **Deterministic, genuinely incremental ingestion** — chunk IDs are derived (`uuid5`) from repo/path/symbol/line, not random, so Qdrant upserts, the parse cache, and the summary cache all correctly recognize unchanged content across separate runs of the same repo
...
- An API key for at least one LLM provider (Anthropic, OpenAI, ZhipuAI's free tier, or an OpenAI-SDK-compatible LiteLLM gateway — the default `MODEL_TYPE`)
...
Dockerfile             Container build for PaaS deployment of webapp/
...
tools/                 Stateless shared utilities — git, GitHub, LLM client, embeddings, Qdrant,
                       BM25, budget guard, event spine, cost estimation
