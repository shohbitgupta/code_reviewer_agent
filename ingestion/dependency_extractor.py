...
class DependencyExtractor:
            ...
            for sym in pf.symbols:
                ...
                if sym.symbol_type in ("function", "method"):
                    for called in sym.calls:
                        edge = self._calls_edge(sym, called, pf, chunk_map, by_name)
                        if edge:
                            edges.append(edge)
                        else:
                            dropped += 1
    ...
    def _resolve_any_symbol(
        ...
        Falls back to O(n) scan if by_name is not provided.
        Prefers chunks in the same file as the caller.
        """
        if by_name is not None:
            candidates = by_name.get(name, [])
        else:
            # Fallback: full scan (only hit on old call-sites without index)
            candidates = [
                c for c in chunk_map.values()
        ...
        return same_file or (candidates[0] if candidates else None)
    ...
    def _module_to_filepath(
        ...
        """
        Convert a dotted Python/JS module path to a relative file path.

        Examples:
            "utils.crypto"         → "utils/crypto.py"
            "./helpers"            → "helpers.js" (relative JS import)
            "django.db.models"     → None (third-party — dropped)
        """
        ...
        # Python dotted path → file
        parts     = module_path.replace("-", "_").split(".")
