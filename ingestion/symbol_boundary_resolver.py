...
from ingestion.models import ParsedFile, ParsedSymbol
...
def _split_symbol(
    ...
    """
    total_parts = _count_parts(sym.end_line - sym.start_line + 1, max_lines, overlap)
    parts: List[ParsedSymbol] = []
    start    = sym.start_line   # 1-indexed
    part_idx = 1

    while start <= sym.end_line:
        end     = min(start + max_lines - 1, sym.end_line)
